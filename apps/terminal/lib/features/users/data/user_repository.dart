...
class UserRepository {
  ...
  Future<Result<List<UserProfile>>> fetchUsers({
    int page = 0,
    int pageSize = 20,
    String? searchQuery,
  }) async {
    return _client.query<List<UserProfile>>(() async {
      ...
      return (response as List<dynamic>).map((e) {
        ...
        // If it has staff details, merge them (priority to driver if somehow both exist, or vice versa)
  ...
  Future<Result<void>> updateUserStatus({
    ...
    return _client.query<void>(() async {
      ...
      await _client.supabase.from('profiles').update(updates).eq('id', userId);
  ...
  Future<Result<void>> createUser({
    ...
    try {
      ...
      if (response.user == null) {
        throw Exception(
          'Failed to create user: No user returned (maybe email exists?)',
        );
      }
      ...
      // We can use the main client for this via updateUserStatus, which returns a Result.
      final updateResult = await updateUserStatus(
        userId: userId,
        role: role,
