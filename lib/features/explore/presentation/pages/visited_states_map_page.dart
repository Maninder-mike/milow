...
class _VisitedStatesMapPageState extends State<VisitedStatesMapPage> {
                  ...
                  child: Center(
                    child: Padding(
                      ...
                      child: SimpleMap(
                        ...
                        // Switch instructions based on toggle
  ...
  Map<String, Color> _getMapColors(bool isDark) {
    ...
    // Only map colors for the active country to keep things clean
    final activeCodes = _showCanada ? _caCodes : _usCodes;

    _stateStats.forEach((code, stats) {
      if (!activeCodes.contains(code)) return;

      Color color = const Color(0xFF007AFF);
      if (stats.visitCount > 5) {
        color = const Color(0xFF003366); // Deep Navy for high activity
      } else if (stats.visitCount > 2) {
        color = const Color(0xFF0056B3); // Rich Blue for medium activity
      } else {
        color = const Color(0xFF007AFF); // Standard Blue for low activity
      }

      // Map package IDs:
      // SMapUnitedStates usually expects just 'ny', 'ca' etc.
      // SMapCanada usually expects just 'on', 'bc' etc.
      // But we will provide both formats to be safe.

      // 1. Raw 2-letter code (common for specific country maps)
      colors[code] = color;

      // 2. Prefixed code (common for world maps)
